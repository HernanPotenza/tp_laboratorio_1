...
{
 setbuf(stdout,NULL);
 setvbuf(stdout, NULL, _IONBF, 0); //Tengo algunos problemas con eclipse, este TP lo hice con codeblocks y me funciona todo perfecto
 setvbuf(stderr, NULL, _IONBF, 0); //Y buscando en internet encontré que con estas líneas tal vez se soluciona lo que me pasa, así que las puse por las dudas
	int opcion;
    float num1;
    float num2;
    float suma;
    ...
    do
    {
        ...
        case 4:
        	...
        	else
        	{
            	...
            	{
            		printf("\ne) El factorial de %.2f es: %d",num1,factorial1);
            	}
            	else
            	{
            	...
            	if(validarNumeroDecimal==0)
            	{
            		printf("\ne) El factorial de %.2f es: %d",num2,factorial2);
            	}
            	else
            	{
